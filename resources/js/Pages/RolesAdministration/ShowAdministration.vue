        ...
        <div class="flex mb-4">
          <div class="w-3/4 h-12">
            <form method="get" @submit.prevent="submit">
              <input v-model="form.name" name="name" placeholder="Busqueda de Rol" type="text" class="w-3/4 my-2 px-3 py-1 border rounded-lg text-xs text-gray-700 focus:outline-none" /> 
            </form>    
          </div>
          <div class="w-1/4 h-12 ">   
            <select v-model="selectedModal.value" class="w-full my-2 px-3 py-1 border rounded-lg text-xs text-gray-700 focus:outline-none" >
              ...
              <option class="py-1" v-for="(option, index) in selectedOptions" :key="index" v-bind:value="index" >{{option}}</option>
            </select>  
          </div>
        </div>   
            ...
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
              <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">              
                <table class="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      <th scope="col" v-for="column in tableColumns" :key="column.name"><jet-title-label>{{column}}</jet-title-label></th>
                      </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">                      
                      <tr v-for="role in roles" :key="role.id"> 
                        <td class="px-6 py-4 text-center"><jet-label>{{role.name}}</jet-label></td>
                        <td class="px-6 py-4 text-center">
                          <jet-label v-for="permission in role.permissions" :key="permission.id">{{permission.description}}, </jet-label></td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <jet-button @click="modalShow=true;selectedRole=role">
                            ...
                            <jet-button @click="modalUpdate=true;selectedRole=role">
                              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                            ...
                            <jet-modify-button @click="modalDelete=true;selectedRole=role">
                              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
   ...
   <jet-dialog-modal :show="modalShow">
      <template v-slot:content>
        <div class="text-center p-5 flex-auto justify-center">
          ...
          <h2 class="text-xl font-bold py-4" v-if="selectedRole"><strong> Rol {{selectedRole.name}}</strong></h2>
          <p class="text-sm text-gray-500 px-8"> Este Rol dispone de los siguientes permisos </p>
      ...
      <template v-slot:footer class="p-3  mt-2 text-center space-x-4 md:block">
        ...
        <jet-danger-button @click="showRoleDetail()">Ir a Deptos</jet-danger-button>    
      </template>
    </jet-dialog-modal>
    ...
    <jet-dialog-modal :show="modalDelete">
      <template v-slot:content>
        <div class="text-center p-5 flex-auto justify-center">
          ...
          <h2 class="text-xl font-bold py-4" v-if="selectedRole"><strong> Role: {{selectedRole.name}}</strong></h2>
      ...
      <template v-slot:footer class="p-3  mt-2 text-center space-x-4 md:block">
        ...
        <jet-danger-button @click="deleteRole()">Dehabilitar</jet-danger-button>    
      </template>
    </jet-dialog-modal>
    ...
    <jet-dialog-modal :show="modalUpdate">
      ...
      <template v-slot:footer class="p-3  mt-2 text-center space-x-4 md:block">
        ...
        <jet-danger-button @click="loadPermissions()"> Modificar</jet-danger-button>   
      </template>
    </jet-dialog-modal>

    <!--Modal Components-->
    <create-role-modal :show="modalCreateRole"></create-role-modal>
    <create-permission-modal :show="modalCreatePermission"></create-permission-modal>
    <modify-permission-modal :show="modalModifyPermission"></modify-permission-modal>
...
import CreateRoleModal from "@/Pages/RolesAdministration/Modals/modalCreateRole";
import CreatePermissionModal from "@/Pages/RolesAdministration/Modals/modalCreatePermission";
import ModifyPermissionModal from "@/Pages/RolesAdministration/Modals/modalModifyPermission";

export default {
    props:{
        roles: Array, tableColumns: Array, selectedOptions: Object,
    },

    data(){  
      return{
        modalShow: false, modalDelete: false, modalUpdate: false, modalCreateRole: false, modalCreatePermission: false, 
        modalModifyPermission: false,
        selectedRole: Object, permissions: Array,
    ...
    components:{
        ...
        JetDialogModal, CreateRoleModal , CreatePermissionModal, ModifyPermissionModal, 
    },
    ...
    watch: {
      'selectedModal.value'(val) {
        ...
        else if(val == 3){ this.openModalModifyPermission()}
      }
    },

    methods:{
      submit: function(){ Inertia.get(route("rolesandpermissions.index"),this.form);},
      openModalCreateRole() {this.modalCreateRole = true;},
      openModalCreatePermission() {this.modalCreatePermission = true;},
      openModalModifyPermission() {this.modalModifyPermission = true;},

      showRoleDetail: function(){     },
      deleteRole: function(){Inertia.delete(route("roles.destroy", { role: this.selectedRole} )); 
