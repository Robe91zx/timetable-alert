                      ...
                      <div class="lg:col-span-2">
                          <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                             <div class="md:col-span-5">
                                  <jet-label>Nombre</jet-label><input v-model="form.name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50"/>
                              </div>                  
                          </div>
...
<script>
    ...
    export default defineComponent({
        ...
        mounted(){
            axios.get(route('permissions.getPermissions')).then((response) =>{/*console.log(response.data);*/ this.permissions = response.data;});
        },

        data(){
             
            return{
                form: { name: null, cpermissions: [] }, permission: Array, 
            }
        },   
