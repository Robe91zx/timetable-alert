  ...
  <div class="flex justify-center h-11/12 items-center bg-gray-200 antialiased">
    <div class="flex flex-col w-full mx-auto rounded-lg border border-gray-300 shadow-xl">
     ...
     <div class="flex flex-row justify-between p-6 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
        <jet-title> Registro de Relacion Facultad - Escuela</jet-title>
      ...
      <div class="flex flex-col px-6 py-5 bg-gray-50">
        <label class="flex flex-col w-full h-32 border-4 border-blue-200 border-dashed hover:bg-gray-100 hover:border-gray-300">
          <div class="flex flex-col items-center justify-center pt-7">
            <upload-icon />
            <p class="pt-1 text-sm tracking-wider text-gray-400 group-hover:text-gray-600" v-if="form.file"> {{form.file.name}}</p>
            <p class="pt-1 text-sm tracking-wider text-gray-400 group-hover:text-gray-600" v-else> Haz Click para buscar tu Archivo</p>
          </div>
          <form @submit.prevent="submit">
            <input type="file" class="opacity-0" @input="form.file = $event.target.files[0]" />
          </form>
        </label>
      ...
      <div class="flex flex-row items-center justify-between p-5 bg-white border-t border-gray-200 rounded-bl-lg rounded-br-lg">
        ...
        <jet-button @click="registerMasive()" :disabled="form.processing"> Guardar</jet-button>
      </div>
    </div>
  </div>
...
<script>
    ...
    export default defineComponent({
        ...
        components: {
            Modal, JetTitle, ExitIcon, UploadIcon, JetButton
        },
        ...
        methods: {
            close: function(){ this.$parent.$parent.closeModalExcelRelationCollege()},
            registerMasive: function(){ Inertia.post(route('facultiesandcolleges.import'), { _method: 'get', file: this.form.file });this.$parent.$parent.closeModalExcelRelationCollege()} ,
        }
    })
</script>