  ...
  <div class="flex justify-center h-11/12 items-center bg-gray-200 antialiased">
    <div class="flex flex-col w-full mx-auto rounded-lg border border-gray-300 shadow-xl">
      <div class="flex flex-row justify-between p-6 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
        <jet-title> Relacion Facultad - Carreras </jet-title>
...
<script>
  ...
  export default defineComponent({
    data(){
      return{
        form: useForm({ faculty_id: null, carreer_id: null})
      }
    },   
        
    mounted() {
      axios.get(route('facultiesandcarreers.create')).then((response)=>{ this.faculties = response.data[0]; this.carreers = response.data[1]; });
    },  
    ...
    methods: {
      ...
      submit(){ this.form.post(route('facultiesandcarreers.store'), {
