...
<div class="flex flex-col w-full mx-auto rounded-lg border border-gray-300 shadow-xl">
  <div class="flex flex-row justify-between p-6 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
    <jet-title> Relacion Facultad - Departamentos </jet-title>
    ...
    <form @submit.prevent="submit">
      <div class="flex flex-col sm:flex-row items-center mb-5 sm:space-x-5">
        ...
        <div class="w-full sm:w-1/2 mt-2 sm:mt-0"> 
          ...
          <div class="italic" if="form.errors.department">{{form.errors.department_id}}</div>  
        </div>
      </div>
    </form>  
...
export default defineComponent({
  data(){
    return{ form: useForm({ faculty_id: null, department_id: null}) }
  },   
        
  mounted() {
    axios.get(route('facultiesanddepartments.create')).then((response)=>{ this.faculties = response.data[0]; this.departments = response.data[1]; });
  },  
  ...
  methods: {
    close: function(){this.$parent.$parent.closeFacultyHasDepartment()},
    submit(){ this.form.post(route('facultiesanddepartments.store'), {
