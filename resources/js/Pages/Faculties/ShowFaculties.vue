...
<app-layout title="Facultades">
        ...
        <tbody class="bg-white divide-y divide-gray-200">                      
          <tr v-for="faculty in faculties" :key="faculty.id"> 
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <jet-button @click="this.modalShow=true;selectedFaculty=faculty"> <jet-show/></jet-button>
              <jet-button @click="this.modalUpdate=true;selectedFaculty=faculty"> <jet-edit/></jet-button>
              <jet-modify-button @click="this.modalDelete=true;selectedFaculty=faculty"> <jet-delete/></jet-modify-button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <faculty-has-department :show="modalRelationFDepartment"></faculty-has-department>
  <excel-faculty-has-department :show="modalExcelRelationFDepartment"></excel-faculty-has-department>
  <faculty-has-college :show="modalRelationFCollege"></faculty-has-college>
  <excel-faculty-has-college :show="modalExcelRelationFCollege"></excel-faculty-has-college>
  <faculty-has-carreer :show="modalRelationFCarreer"></faculty-has-carreer>
  <excel-faculty-has-carreer :show="modalExcelRelationFCarreer"></excel-faculty-has-carreer>
...
export default {
  ...
  data(){  
    return{
      modalShow: false, modalUpdate: false, modalDelete: false, modalRegisterFaculty: false, modalEditFaculties: false, modalExcelRegister: false,   
      modalRelationFDepartment: false, modalExcelRelationFDepartment: false , modalRelationFCollege: false, modalExcelRelationFCollege: false,
      modalRelationFCarreer: false, modalExcelRelationFCarreer: false,  modalRelationFSubject: false, modalExcelRelationSubject: false,
  ...
  components:{
    ...
    RegisterFaculty , EditFaculties, ExcelRegister, 
    FacultyHasDepartment, ExcelFacultyHasDepartment, FacultyHasCollege, ExcelFacultyHasCollege,
    FacultyHasCarreer, ExcelFacultyHasCarreer, FacultyHasSubject, ExcelFacultyHasSubject,
  },

  watch: {
    'selectedModal.value'(val) {
    //console.log(this.selectedModal.value)
    if(val == 1){ this.openRegisterFaculty()}
    else if(val == 2){ this.openFacultyHasDepartment()}
    else if(val == 3){ this.openExcelFacultyHasDepartments()}        
    else if(val == 4){ this.openFacultyHasCollege()}        
    else if(val == 5){ this.openExcelFacultyHasColleges()}
    else if(val == 6){ this.openFacultyHasCarreer()} 
    else if(val == 7){ this.openExcelFacultyHasCarreers()} 
    else if(val == 8){ this.openFacultyHasSubject()} 
    else if(val == 9){ this.openExcelFacultyHasSubjects()} 
    else if(val == 10){ this.openEditFaculties()} 
    }
  },
  ...
  methods:{
    ...
    deleteFaculty: function(){Inertia.delete(route("faculties.destroy", { faculty: this.selectedFaculty} )); this.modalDelete=false},
    updateFaculty: function(){Inertia.get(route("faculties.edit", { faculty: this.selectedFaculty} )); this.modalUpdate=false},
    showDepartmentsOf: function(){Inertia.get(route("faculties.showDepartmentsandColleges", {faculty: this.selectedFaculty} )); this.modalShow=false}, 
    showCarreersOf: function(){Inertia.get(route("faculties.showCarreers", {faculty: this.selectedFaculty} )); this.modalShow=false}, 
    showSubjectsOf: function(){Inertia.get(route("faculties.showSubjects", {faculty: this.selectedFaculty} )); this.modalShow=false}, 
  
    openRegisterFaculty() {this.modalRegisterFaculty = true}, closeRegisterFaculty() {this.modalRegisterFaculty = false},
    openEditFaculties(){this.modalEditFaculties = true}, closeEditFaculties() {this.modalEditFaculties = false},
    openExcelRegister() {this.modalExcelRegister = true}, closeExcelRegister() {this.modalExcelRegister = false},
    openFacultyHasDepartment() {this.modalRelationFDepartment = true}, closeFacultyHasDepartment() {this.modalRelationFDepartment = false},
    openExcelFacultyHasDepartments() {this.modalExcelRelationFDepartment = true}, closeExcelFacultyHasDepartments() {this.modalExcelRelationFDepartment = false},
    openFacultyHasCollege() {this.modalRelationFCollege = true}, closeFacultyHasCollege() {this.modalRelationFCollege = false},
    openExcelFacultyHasColleges() {this.modalExcelRelationFCollege = true}, closeExcelFacultyHasColleges() {this.modalExcelRelationFCollege = false},
    openFacultyHasCarreer() {this.modalRelationFCarreer = true}, closeFacultyHasCarreer() {this.modalRelationFCarreer = false},
    openExcelFacultyHasCarreers() {this.modalExcelRelationFCarreer = true}, closeExcelFacultyHasCarreers() {this.modalExcelRelationFCarreer = false},
    openFacultyHasSubject() {this.modalRelationFSubject = true}, closeFacultyHasSubject() {this.modalRelationFSubject = false},
    openExcelFacultyHasSubjects() {this.modalExcelRelationFSubject = true}, closeExcelFacultyHasSubjects() {this.modalExcelRelationFSubject = false},
