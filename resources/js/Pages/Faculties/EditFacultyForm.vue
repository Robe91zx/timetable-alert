    ...
    <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
      ...
      <div class="lg:col-span-2">
        ...
        <div class="inline-flex items-end py-8">
          ...
          <p class="font-semibold text-gray-600 cursor-pointer"> <Link :href="route('faculties.index')"> Regresar</Link> </p>
         </div>   
      </div>
    </div>
    </div>
...
export default {
  components:{
    AppLayout, Link, JetLabel, JetInput,  JetButton, JetModifyButton
  },
    
  props:{ faculty: Object},
    
  data(){
    return{ 
      form: useForm({ name: this.$props.faculty.name, vcode: this.$props.faculty.vcode, code: this.$props.faculty.code, email : this.$props.faculty.email,
                      phone: this.$props.faculty.phone, website: this.$props.faculty.website
      })   
    }      
  },

  methods:{
    submit(){ this.form.put( route('faculties.update', this.$props.faculty)) },
  }
}
</script>