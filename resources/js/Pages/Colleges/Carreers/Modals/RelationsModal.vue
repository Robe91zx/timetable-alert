...
<div class="flex flex-col w-full mx-auto rounded-lg border border-gray-300 shadow-xl">
  <div class="flex flex-row justify-between p-6 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
    <jet-title> Relacion Escuelas - Carreras </jet-title>
    ...
    <form @submit.prevent="submit">
      <div class="flex flex-col sm:flex-row items-center mb-5 sm:space-x-5">
        <div class="w-full sm:w-1/2">
          <p class="mb-2 font-semibold text-gray-700">Escuelas</p>
          ...
          <div class="italic" if="form.errors.college_id">{{form.errors.college_id}}</div>  
        </div>
...
export default defineComponent({
  data(){
    return{ form: useForm({ college_id: null, carreer_id: null}) }
  },   
        
  mounted() {
    axios.get(route('collegesandcarreers.create')).then((response)=>{ this.colleges = response.data[0]; this.carreers = response.data[1]; });
  },  
  ...
  methods: {
    close: function(){this.$parent.$parent.closeCollegeHasCarreer()},
    submit(){ this.form.post(route('collegesandcarreers.store'), {
